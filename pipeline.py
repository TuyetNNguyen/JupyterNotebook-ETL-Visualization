...
class ScoreTask(luigi.Task):
    ...
    """
    tweet_file = luigi.Parameter()
    cities_file = luigi.Parameter(default='cities.csv') #Added
    output_file = luigi.Parameter(default='scores.csv')

    def requires(self):
        # I require the output from all tasks, since the output files are present, it won't run again (?)
    ...
    def run(self):
        ...
        # Because at earlier stages, the closest city information was never save (only the OHE encoding), I will do it again here
        # There are more efficient ways to keep track of the closest city column rather than compute it again, but this will do.
        tweets_clean_df['closest_city'] = tweets_clean_df['tweet_coord'].map(ast.literal_eval).map(lambda coords: closest_node(coords, cities_df[['latitude', 'longitude']])).map(lambda ind: cities_df['name'][ind])
        ...
        scores_df.to_csv(self.output().path)
